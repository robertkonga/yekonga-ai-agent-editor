    ...
    <div class="flex h-full min-h-0 w-full flex-col bg-slate-900 font-sans text-[#cccccc]">
        ...
        <div class="flex items-center border-b border-slate-800/60 px-5 py-4">
            <div class="flex items-center justify-between flex-1">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <h1 class="text-[13px] font-semibold leading-none tracking-wide text-[#e8e8e8]">Open Workspace <i class="ye ye-star ye-xs"></i></h1>
                        <p class="mt-1 text-[11px] text-[#858585]">Select a recent workspace or open a folder</p>
                    </div>
                </div>
                <button
                    class="flex cursor-pointer items-center gap-1.5 rounded-full bg-blue-500/20 px-3 py-1.5 text-[12px] font-medium text-white transition-colors hover:bg-blue-600/30 focus-visible:outline focus-visible:outline-blue-500 focus-visible:outline-offset-1"
                    @click="openWorkspace">
        ...
        <div class="flex-1 overflow-y-auto">
            <div class="w-1/2 mx-auto ">
                ...
                <!-- List -->
                <div class="min-h-0 px-4 pb-2">
                    
                    <!-- Empty state -->
                    <div v-if="filtered.length === 0" class="flex flex-col items-center justify-center gap-3 py-10 text-center min-h-80">
                        <svg class="text-slate-700/60 size-20"  viewBox="0 0 32 32" fill="none">
                        ...
                        <template v-else>
                            <p class="text-[12px] text-[#555]">
                                No workspaces match <span class="text-[#858585]">"{{ search }}"</span>
                            </p>
                        </template>
                    </div>
                    ...
                    <template v-else>
                        <!-- Pinned -->
                        <div v-if="pinned.length > 0" class="space-y-2 mb-1">
                            <div class="mb-1 px-0 pt-1 text-[10px] font-semibold uppercase tracking-widest text-[#555]">
                                Pinned
                            </div>
                            <WorkspaceRow v-for="ws of pinned" :key="ws.id" :workspace="ws" :hovered="hoveredId === ws.id"
                                @mouseenter="hoveredId = ws.id" @mouseleave="hoveredId = null" @open="emit('open', ws)"
                                @remove="emit('removeRecent', ws.id)" />
                        </div>
    
                        <!-- Recent -->
                        <div v-if="recent.length > 0" class="space-y-2">
                            <div class="px-0 pt-1 text-[10px] font-semibold uppercase tracking-widest text-[#555]">
...
import WorkspaceRow from './components/Workspace/WorkspaceRow.vue'
import { useWorkspace, type Workspace } from '@/store/workspace.ts'
import { OpenWorkspaceDialog } from '@wails/yekonga-builder/service.ts'
...
const recentWorkspaces = computed<Workspace[]>(()=>{
    ...
    list = list.sort((a, b) => {
        let now = window.dateToNumber(new Date());
        let lastA = now - window.dateToNumber(a.lastOpened);
        let lastB = now - window.dateToNumber(b.lastOpened);
