...
import { AgentChat, LoadConfigFromFile, SaveConfigToFile } from '@wails/yekonga-builder/service.ts'
...
onMounted(async () => {
    ...
    Events.On('scaffold:progress', (p: any) => {
        if (p.error) {
...
onUnmounted(() => {
    ...
    Events.Off('scaffold:progress')
    Events.Off('agent:message')
    Events.Off('agent:tool')
