        ...
        <template #default>
            <div class="min-h-40 max-h-80 overflow-y-auto flex flex-col gap-2 py-2">
                ...
                <div v-for="session in store.active?.sessions" :key="session.id" 
                     ...
                     :class="{'bg-slate-800 border-l-2 border-blue-500': store.active?.sessionId === session.id}">
                    <div class="flex items-center gap-2 text-xs text-gray-300">
                        <span class="w-1.5 h-1.5 rounded-full" :class="store.active?.sessionId === session.id ? 'bg-blue-500' : 'bg-gray-500'"></span>
