...
func (a *Service) GetGitLog(path string) ([]GitCommit, error) {
	if path == "" {
...
func (a *Service) GetGitCommitDetail(path string, hash string) (*GitCommitDetail, error) {
	if path == "" {
		path = a.agent.ActivePath
	}
	gitRoot, err := findGitRoot(path)
...
func (a *Service) SearchInFiles(path string, query string) ([]SearchResult, error) {
	if path == "" {
		path = a.agent.ActivePath
	}
	if query == "" {
...
func (a *Service) ReplaceInFiles(path string, searchText string, replaceText string) ([]ReplaceResult, error) {
	if path == "" {
		path = a.agent.ActivePath
	}
	if searchText == "" {
