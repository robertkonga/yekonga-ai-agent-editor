...
func NewAgent(apiKeys ApiKeys, ollamaHost string, app *application.App) *Agent {
...
func (a *Agent) Emit(p fileTypes.ScaffoldProgress) {
	a.app.Event.Emit("scaffold:progress", p)
}
...
func (a *Agent) AgentChat(sessionID string, userMessage string, providerName string, modelName string) error {
	...
	for {
		...
		if err != nil {
			a.app.Event.Emit("agent:error", err.Error())
			return err
		}
		...
		if resp.Content != "" {
			...
			a.app.Event.Emit("agent:message", resp.Content)
		}
		...
		if len(resp.ToolCalls) == 0 {
			...
			a.app.Event.Emit("agent:done", nil)
			return nil
		}
		...
		for _, call := range resp.ToolCalls {
			a.app.Event.Emit("agent:tool", map[string]string{
				"name":  call.Name,
